...
function listarBytesRecebidos(id) {
    ...
    var instrucao2 = `SELECT TOP 25 bytesRecv,,horarioLeitura FROM dadosRede INNER JOIN rede ON fkRede = idRede WHERE fkServidor = ${id}  ORDER BY horarioLeitura DESC;`;
...
function  tempoRealBytesRecv(id) {
    ...
    var instrucao2 = `SELECT TOP 1 bytesRecv,horarioLeitura FROM dadosRede INNER JOIN rede ON fkRede = idRede WHERE fkServidor = ${id}  ORDER BY horarioLeitura DESC;`;
    return database.select(instrucao,instrucao2);
}
