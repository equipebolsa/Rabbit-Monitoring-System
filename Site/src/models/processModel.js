...
function listarAguardando(nome) {
    var instrucao = `select * from waitlist where nome like '%${nome}%' order by nome;`;
...
function kpiDeath(nome) {
    var instrucao = `select count(dataHora) as qtdMorte, dataHora from deathLog where nome like '${nome}' group by dataHora order by dataHora;`;
    return database.select(instrucao);
}
function countDeath() {
